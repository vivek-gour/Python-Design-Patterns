...
But before talking about the self keyword (which is actually not a python keyword or any special literal), first let’s
...
that we should expect from a class variable. Let’s find about instance variables:
...
but what that parameter represents will always be the same. And it’s a good idea for sticking with self as it’s the
convention.
...
that method, we do not pass anything for self as arguments. How come this does not give errors? What’s going on behind
...
Now with all things are being reminded (instance/class variables and methods), let’s talk about the use of self
in python ( — finally).

self — intuition
Some of you may have got it by now, or some may have got it partially; anyway, the self in python represents or
points the instance which it was called. Let’s clarify this with an example:
...
Being said that, let’s look at above example. There we have created obj1 and are calling the instance method
...
value 6? Ok, now I think you got it. That’s the job of self. Behind the scene, in every instance method call,
